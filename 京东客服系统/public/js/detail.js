 	...
 	oShow_box.onmouseover=function(){
 		for(var i=0; i<aImg.length; i++){
 			if(aImg[i].style.display=="block"){
 				zoom_box.children[0].src="images/bbs"+(i+1)+".jpg";
 			}		
 		}
 		...
 		for(var j=0; j<aLi.length; j++){
 			...
 			aLi[j].onclick = function(){
 				...
 				for(var k=0; k<aLi.length; k++){
 					// console.log(j,k);
 					if(aLi[k].getElementsByClassName('foot')[0]){
 						var oImage=aLi[k].getElementsByClassName('foot')[0];
 						aLi[k].removeChild(oImage);
 					}
 					aLi[k].className="";
 				}
